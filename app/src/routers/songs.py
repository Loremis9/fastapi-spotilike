...
async def create_song_by_id(albums_id : UUID, songs: songCreate , db: Session = Depends(get_db)):
...
async def modify_types_by_id(genre_id : UUID,schema: TypeModify, db: Session = Depends(get_db)):
    return service_type.put_type(db, genre_id, schema)

@router.delete("/genre/{genre_id}", response_model=None)
async def delete_song_by_id(genre_id : UUID, db: Session = Depends(get_db), user = Annotated[User,Depends(service_user_session.get_current_user)]):
